...
function VisualizeArray({ len, speed }) {
	...
	for (
		let i = len - 1;
		i > 0;
		i-- //Change 10 to len after testing
	) {
	...
	const heapSort = () => {
		...
		for (let i = 0; i < animations.length; i++) {
			const arrayBars = document.getElementsByClassName('array-bar');
			const isColorChange =
				animations[i][0] === 'Comparing' ||
				animations[i][0] === 'Done-Comparing';
			if (isColorChange) {
	...
	return (
		<div className='container'>
			...
			<div>
				<button onClick={() => setSelectedAlgo('')}>Reset</button>
			</div>
			{(!selectedAlgo && <Personal />) ||
				(selectedAlgo === 'BubbleSort' && <BubbleSortInformation />) ||
				(selectedAlgo === 'MergeSort' && <MergeSortInformation />) ||
				(selectedAlgo === 'InsertionSort' && (
					<InsertionSortInformation />
				)) ||
				(selectedAlgo === 'QuickSort' && <QuickSortInformation />) ||
				(selectedAlgo === 'HeapSort' && <HeapSortInformation />)}
		</div>
	);
}
